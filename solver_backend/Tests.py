import unittest
from Solvers import BruteForceSolver, OrderSolver, MinizincSolver, DynamicSolver, GeneticSolver, OrderSolver
...
class TestSolver(unittest.TestCase):
    ...
    def test_result_length(self):
        ...
        solver = OrderSolver()
        result = solver.solve(self. large_matrix)
        self.assertEqual(len(result), len(self.large_matrix) + 1)
    ...
    def test_result_are_all_different_except_last_element(self):
        ...
        solver = OrderSolver()
        result = solver.solve(self.large_matrix)[:-1]  # remove the last element that is always equal to 0
    ...
    def test_correct_last_element_if_starting_city_different_than_ending(self):
        ...
        solver = OrderSolver()
        result = solver.solve(self.large_matrix, startCity=0, endCity=1)

        self.assertEqual(len(result), len(self.large_matrix))
        self.assertEqual(result[0], 0)
    ...
    def test_returns_to_correct_city_on_provided_start_city_and_end_city(self):
        ...
        solver = OrderSolver()
        result = solver.solve(self.large_matrix, startCity=1, endCity=1)
