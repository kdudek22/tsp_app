...
class AbstractSolver(ABC):
    ...
    @staticmethod
    def assertAllUnique(ordering):
        counter = Counter(ordering)
        for num in counter:
            assert (counter[num] == 1), "The given ordering is not unique"

    @staticmethod
    def calculateCostFromMatrix(matrix: [[float]], ordering: [int]) -> float:
        ...
        AbstractSolver.assertAllUnique(ordering)
...
class OrderSolver(AbstractSolver):
    ...
    def solve(self, matrix: [[float]], startCity: int = 0, endCity: int = 0) -> list[int]:
        if startCity == endCity:
            return [startCity] + [i for i in range(len(matrix)) if i != startCity] + [startCity]
        return [startCity] + [i for i in range(len(matrix)) if i not in [startCity, endCity]] + [endCity]
